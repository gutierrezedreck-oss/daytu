...
3. **Multi-tab token refresh race.** Originally a `navigatorLock` deadlock — two tabs shared the auth-refresh lock and could wedge each other during session recovery. The processLock switch (bug #7 fix) made that impossible: processLock is an in-memory promise chain that's per-JS-context, so different tabs no longer share any lock. The new shape under processLock: two tabs can attempt token refreshes independently with no cross-tab serialization. If both refresh in overlapping windows, one tab consumes the refresh token first; the other tab's in-flight refresh fails with "refresh token already used", surfacing as a transient auth error in the losing tab. Severity: low — refresh windows are narrow (a few seconds around token expiry, ~hourly) and most users don't keep two daytu tabs open simultaneously; the losing tab recovers by reloading, which picks up the freshly-refreshed session from storage. Mitigations if it becomes a real problem: build a BroadcastChannel-based leader-election coordinator (one tab does refreshes, others wait), or accept it and surface a "Session expired, please reload" toast on refresh failure.
...
Issues #3 and #8 remain open. Calendar app + auth core are fully usable.
...
5. **Decide on issue #3 (multi-tab refresh race).** Either build a BroadcastChannel-based leader-election coordinator so only one tab handles token refreshes, or accept the rare losing-tab error and surface a "Session expired, please reload" toast on refresh failure. The original `navigatorLock` deadlock concern is moot post-bug-#7 (processLock).
