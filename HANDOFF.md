...
Issues #3, #5, and #8 remain open. Calendar app + auth core are fully usable.
...
7. **Data layer migration — events.** Step-by-step plan. Steps 1 and 2 done; 3 and 4 open.
   ...
   - **Step 3: optimistic write rewires.** `addEvent` / `updateEvent` / `deleteEvent` / `duplicateEvent` become Supabase-backed with optimistic local state and revert-on-error. App.jsx writes still go to localStorage today.
   - **Step 4: cleanup.** Remove `events` from the persist effect's saved blob and dependency array (it currently saves the whole app state including events). Update calendar-delete fallback. Update `setEvents([])` reset paths.
