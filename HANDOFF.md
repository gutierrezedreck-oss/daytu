...
7. **`loadProfile` hangs on plain reload while signed in — status: to investigate.** Symptom: reloading the tab while already signed in lands on the "Something went wrong" error screen after the 8s watchdog fires. Console trace: `SIGNED_IN` event fires → `[auth] loadProfile start` logs → 8s pass → `[auth] watchdog fired — stuck in loading for 8s`. The `profiles` SELECT never resolves. Same hang pattern as the multi-tab nav-lock race (#3) but reproduces with a single tab and no email-link involvement, so the root cause is triggered differently. Suspect a stale-token or auth-lock state issue carried over from a previous session lifecycle. Next session: capture the exact network state of the in-flight `profiles` request when it stalls, and check whether `getSession()` immediately before `loadProfile` already shows degraded state.
