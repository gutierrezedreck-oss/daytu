-- ============================================================================
-- daytu — major_events.pinned
--
-- Adds a pinned boolean to major_events. Local clients have tracked
-- per-major-event pinned state in localStorage (me.pinned per object); the
-- one-time client migration (Phase 2) preserves it by writing through to
-- this column. After migration, pinned syncs cross-device — same trade-off
-- as events.pinned: the owner's pin state is authoritative for shared major
-- events.
--
-- major_events_for_viewer() must be dropped + recreated because we're
-- changing the shape of the returns table (CREATE OR REPLACE can't change
-- return type). Body is otherwise identical to the original in
-- 20260424120000_social_layer.sql, with `pinned boolean` in the returns
-- table and `m.pinned` in the projection.
-- ============================================================================
...
returns table (
  ...
  notes text, location text, url text, visibility text, pinned boolean,
...
as $$
  ...
  select
    ...
    m.notes, m.location, m.url, m.visibility, m.pinned,
